...
function destroyItem(player, target, toPosition)
	...
	if math.random(7) == 1 then
		...
		if target.itemid == 7585 then
			local eggStorage = player:getStorageValue(PlayerStorageKeys.TibiaTales.AgainstTheSpiderCult)
			if eggStorage >= 1 and eggStorage < 5 then
				player:setStorageValue(PlayerStorageKeys.TibiaTales.AgainstTheSpiderCult, math.max(1, eggStorage) + 1)
			end
...
function onUseRope(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if table.contains(ropeSpots, tile:getGround():getId()) or tile:getItemById(14435) then
		...
		if target.itemid == 8592 then
			if player:getStorageValue(PlayerStorageKeys.RookgaardTutorialIsland.tutorialHintsStorage) < 22 then
...
function onUseShovel(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif target.itemid == 20230 then -- swamp digging
		if player:getStorageValue(PlayerStorageKeys.SwampDiggingTimeout) <= os.time() then
			...
			player:setStorageValue(PlayerStorageKeys.SwampDiggingTimeout, os.time() + 7 * 24 * 60 * 60)
	...
	elseif target.itemid == 8579 and player:getStorageValue(PlayerStorageKeys.RookgaardTutorialIsland.tutorialHintsStorage) < 20 then
	...
	-- Gravedigger Quest
	elseif target.actionid == 4654 and player:getStorageValue(PlayerStorageKeys.GravediggerOfDrefia.Mission49) == 1 and player:getStorageValue(PlayerStorageKeys.GravediggerOfDrefia.Mission50) < 1 then
	...
	elseif target.actionid == 4668 and player:getStorageValue(PlayerStorageKeys.GravediggerOfDrefia.Mission67) == 1 and player:getStorageValue(PlayerStorageKeys.GravediggerOfDrefia.Mission68) < 1 then
...
function onUsePick(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.itemid == 3621 and target.actionid == 12026 then
		local missionProgress, pickAmount = player:getStorageValue(PlayerStorageKeys.TheIceIslands.Mission02), player:getStorageValue(PlayerStorageKeys.TheIceIslands.PickAmount)
		if missionProgress < 1 or pickAmount >= 3 or player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) ~= 3 then
			return false
		end

		player:setStorageValue(PlayerStorageKeys.TheIceIslands.PickAmount, math.max(0, pickAmount) + 1)
		player:setStorageValue(PlayerStorageKeys.TheIceIslands.Mission02, missionProgress + 1) -- Questlog The Ice Islands Quest, Nibelor 1: Breaking the Ice

		if pickAmount >= 2 then
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Questline, 4)
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Mission02, 4) -- Questlog The Ice Islands Quest, Nibelor 1: Breaking the Ice
		end
	...
	if target.itemid == 1304 then
		...
		elseif target.actionid == 50058 then
			local stoneStorage = Game.getStorageValue(GlobalStorageKeys.NaginataStone)
			if stoneStorage ~= 5 then
				Game.setStorageValue(GlobalStorageKeys.NaginataStone, math.max(0, stoneStorage) + 1)
			elseif stoneStorage == 5 then
				target:remove()
				Game.setStorageValue(GlobalStorageKeys.NaginataStone)
			end
	...
	if target.actionid == 50090 then
		if player:getStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.WayToBeregar) == 1 then
	...
	if target.actionid == 50127 then
		if player:getStorageValue(PlayerStorageKeys.QuestChests.FirewalkerBoots) == 1 then
			return false
		end
		target:remove()
...
function onUseCrowbar(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.uid == 3071 then
		if player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe01) < 1 then
			doSetMonsterOutfit(player, 'skeleton', 3 * 1000)
			fromPosition:sendMagicEffect(CONST_ME_ENERGYHIT)
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe01, 1)
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01, player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01) + 1) -- StorageValue for Questlog 'Mission 01: Something Rotten'
	...
	elseif target.uid == 3072 then
		if player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe02) < 1 then
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe02, 1)
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01, player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01) + 1) -- StorageValue for Questlog 'Mission 01: Something Rotten'
			local position = player:getPosition()
			for x = -1, 1 do
				for y = -1, 1 do
	...
	elseif target.uid == 3073 then
		if player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe03) < 1 then
			player:say('You have used the crowbar on a grate.', TALKTYPE_MONSTER_SAY)
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe03, 1)
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01, player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01) + 1) -- StorageValue for Questlog 'Mission 01: Something Rotten'
		end

	elseif target.uid == 3074 then
		if player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe04) < 1 then
			doSetMonsterOutfit(player, 'bog raider', 5 * 1000)
			player:say('You have used the crowbar on a knot.', TALKTYPE_MONSTER_SAY)
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.SewerPipe04, 1)
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01, player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.Mission01) + 1) -- StorageValue for Questlog 'Mission 01: Something Rotten'
		end
	...
	elseif target.actionid == 100 then
		...
		if target.itemid == 2593 then
			if player:getStorageValue(PlayerStorageKeys.postman.Mission02) == 1 then
				player:setStorageValue(PlayerStorageKeys.postman.Mission02, 2)
				toPosition:sendMagicEffect(CONST_ME_MAGIC_BLUE)
			end

		-- The Ape City - Mission 7
		elseif target.itemid == 5539 then
			local cStorage = player:getStorageValue(PlayerStorageKeys.TheApeCity.Casks)
			if cStorage < 3 then
				player:setStorageValue(PlayerStorageKeys.TheApeCity.Casks, math.max(0, cStorage) + 1)
	...
	elseif target.actionid == 12566 and player:getStorageValue(PlayerStorageKeys.secretService.TBIMission06) == 1 then
		local yellPosition = Position(32204, 31157, 8)
		if player:getOutfit().lookType == 137 then -- amazon lookType
			player:setStorageValue(PlayerStorageKeys.secretService.TBIMission06, 2)
...
function onUseSpoon(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.itemid == 388 then
		if player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) >= 21 then
			if player:getStorageValue(PlayerStorageKeys.TheIceIslands.SulphurLava) < 1 then
				player:addItem(8301, 1)
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.SulphurLava, 1)
	...
	elseif target.itemid == 4184 then
		if player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) >= 21 then
			if player:getStorageValue(PlayerStorageKeys.TheIceIslands.SporesMushroom) < 1 then
				player:addItem(7251, 1)
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.SporesMushroom, 1)
				toPosition:sendMagicEffect(CONST_ME_MAGIC_RED)
	...
	elseif target.itemid == 8573 then
		if player:getStorageValue(PlayerStorageKeys.WhatAFoolishQuest.Questline) ~= 21
				or player:getStorageValue(PlayerStorageKeys.WhatAFoolishQuest.InflammableSulphur) == 1 then
			return false
		end

		player:setStorageValue(PlayerStorageKeys.WhatAFoolishQuest.InflammableSulphur, 1)
...
function onUseKitchenKnife(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.itemid == 7261 then
		if player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) >= 21 then
			if player:getStorageValue(PlayerStorageKeys.TheIceIslands.FrostbiteHerb) < 1 then
				player:addItem(7248, 1)
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.FrostbiteHerb, 1)
				toPosition:sendMagicEffect(CONST_ME_MAGIC_BLUE)
	...
	elseif target.itemid == 2733 then
		if player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) >= 21 then
			if player:getStorageValue(PlayerStorageKeys.TheIceIslands.FlowerCactus) < 1 then
				player:addItem(7245, 1)
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.FlowerCactus, 1)
	...
	elseif target.itemid == 4017 then
		if player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) >= 21 then
			if player:getStorageValue(PlayerStorageKeys.TheIceIslands.FlowerBush) < 1 then
				player:addItem(7249, 1)
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.FlowerBush, 1)
	...
	elseif target.itemid == 7480 then
		if player:getStorageValue(PlayerStorageKeys.WhatAFoolishQuest.EmperorBeardShave) == 1 then
			player:say('God shave the emperor. Some fool already did it.', TALKTYPE_MONSTER_SAY)
			return true
		end

		player:setStorageValue(PlayerStorageKeys.WhatAFoolishQuest.EmperorBeardShave, 1)
	...
	elseif target.itemid == 4008 then
		if player:getStorageValue(PlayerStorageKeys.WhatAFoolishQuest.Questline) ~= 22
				or player:getStorageValue(PlayerStorageKeys.WhatAFoolishQuest.SpecialLeaves) == 1 then
			return false
		end

		player:setStorageValue(PlayerStorageKeys.WhatAFoolishQuest.SpecialLeaves, 1)
