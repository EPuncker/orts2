function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if(item.uid == 3084) then
		if(player:getStorageValue(PlayerStorageKeys.InServiceofYalahar.MatrixReward) < 1) then
			player:setStorageValue(PlayerStorageKeys.InServiceofYalahar.MatrixReward, 1)
