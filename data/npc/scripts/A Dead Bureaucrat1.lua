...
local config = {
	[VOCATION_SORCERER] = "wand",
	[VOCATION_DRUID] = "rod",
	[VOCATION_PALADIN] = "bow",
	[VOCATION_KNIGHT] = "sword"
}
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "pumin") then
		if npcHandler.topic[cid] == 0 and player:getStorageValue(PlayerStorageKeys.PitsOfInferno.Pumin) < 1 then
			npcHandler:say("Sure, where else. Everyone likes to meet my master, he is a great demon, isn't he? Your name is ...?", cid)
			npcHandler.topic[cid] = 1
		elseif npcHandler.topic[cid] == 3 then
			player:setStorageValue(PlayerStorageKeys.PitsOfInferno.Pumin, 1)
	...
	elseif msgcontains(msg, "411") then
		if player:getStorageValue(PlayerStorageKeys.PitsOfInferno.Pumin) == 3 then
			npcHandler:say("Form 411? You need Form 287 to get that! Do you have it?", cid)
			npcHandler.topic[cid] = 4
		elseif player:getStorageValue(PlayerStorageKeys.PitsOfInferno.Pumin) == 5 then
			npcHandler:say("Form 411? You need Form 287 to get that! Do you have it?", cid)
			npcHandler.topic[cid] = 5
		end
	elseif msgcontains(msg, "no") then
		if npcHandler.topic[cid] == 4 then
			player:setStorageValue(PlayerStorageKeys.PitsOfInferno.Pumin, 4)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 5 then
			player:setStorageValue(PlayerStorageKeys.PitsOfInferno.Pumin, 6)
