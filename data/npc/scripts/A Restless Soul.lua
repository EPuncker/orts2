...
local function greetCallback(cid)
	...
	if player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) < 37 then
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "story") then
		...
		if player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) == 37 then
			...
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Questline, 38)
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Mission10, 2) -- Questlog The Ice Islands Quest, Formorgar Mines 2: Ghostwhisperer
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Mission11, 1) -- Questlog The Ice Islands Quest, Formorgar Mines 3: The Secret
		end
	end
	return true
end
