...
local function greetCallback(cid)
	...
	if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 1 or player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) > 3 then
		npcHandler:setMessage(MESSAGE_GREET, "Whatcha do in my place?")
	elseif player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 2 and player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddonWaitTimer) < os.time() then
		npcHandler:setMessage(MESSAGE_GREET, "You back. You know, you right. Brother is right. Fist not always good. Tell him that!")
		player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 3)
	end
	return true
end
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "gelagos") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 4 then
	...
	elseif msgcontains(msg, "fighting spirit") then
		if npcHandler.topic[cid] == 6 then
			...
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 5)
			npcHandler.topic[cid] = 0
		end
	elseif msgcontains(msg, "present") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 11 then
	...
	elseif msgcontains(msg, "ore") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 12 then
			npcHandler:say("You bring 100 iron ore?", cid)
			npcHandler.topic[cid] = 8
		end
	elseif msgcontains(msg, "iron") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 13 then
			npcHandler:say("You bring crude iron?", cid)
			npcHandler.topic[cid] = 9
		end
	elseif msgcontains(msg, "fangs") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 14 then
			npcHandler:say("You bring 50 behemoth fangs?", cid)
			npcHandler.topic[cid] = 10
		end
	elseif msgcontains(msg, "leather") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 15 then
			npcHandler:say("You bring 50 lizard leather?", cid)
			npcHandler.topic[cid] = 11
		end
	elseif msgcontains(msg, "axe") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 16 and player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddonWaitTimer) < os.time() then
			npcHandler:say("Axe is done! For you. Take. Wear like me.", cid)
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 17)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 5 then
			...
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 2)
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddonWaitTimer, os.time() + 60 * 60) -- 1 hour
		...
		elseif npcHandler.topic[cid] == 7 then
			...
			npcHandler.topic[cid] = 0
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 12)
		elseif npcHandler.topic[cid] == 8 then
			if player:removeItem(5880, 100) then
				npcHandler:say("Good! Now bring crude iron.", cid)
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 13)
				npcHandler.topic[cid] = 0
			end
		elseif npcHandler.topic[cid] == 9 then
			if player:removeItem(5892, 1) then
				npcHandler:say("Good! Now bring 50 behemoth fangs.", cid)
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 14)
				npcHandler.topic[cid] = 0
			end
		elseif npcHandler.topic[cid] == 10 then
			if player:removeItem(5893, 50) then
				npcHandler:say("Good! Now bring 50 lizard leather.", cid)
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 15)
				npcHandler.topic[cid] = 0
			end
		elseif npcHandler.topic[cid] == 11 then
			if player:removeItem(5876, 50) then
				...
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 16)
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddonWaitTimer, os.time() + 2 * 60 * 60) -- 2 hours
				npcHandler.topic[cid] = 0
			end
		end
	end
