...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		if player:getStorageValue(PlayerStorageKeys.TibiaTales.ultimateBoozeQuest) == 2 and player:removeItem(7495, 1) then
			player:setStorageValue(PlayerStorageKeys.TibiaTales.ultimateBoozeQuest, 3)
		...
		elseif player:getStorageValue(PlayerStorageKeys.TibiaTales.ultimateBoozeQuest) < 1 then
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			...
			player:setStorageValue(PlayerStorageKeys.TibiaTales.ultimateBoozeQuest, 1)
