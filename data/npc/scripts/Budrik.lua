...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		if player:getStorageValue(PlayerStorageKeys.toOutfoxAFoxQuest) < 1 then
			npcHandler:say({
		...
		elseif player:getStorageValue(PlayerStorageKeys.toOutfoxAFoxQuest) == 1 then
			if player:removeItem(7497, 1) then
				player:setStorageValue(PlayerStorageKeys.toOutfoxAFoxQuest, 2)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			player:setStorageValue(PlayerStorageKeys.toOutfoxAFoxQuest, 1)
