...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		if player:getStorageValue(PlayerStorageKeys.CaptainHaba) <= 1 then
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			...
			player:setStorageValue(PlayerStorageKeys.CaptainHaba, 2)
	...
	elseif msgcontains(msg, "bait") then
		if player:getStorageValue(PlayerStorageKeys.CaptainHaba) == 2 then
			if player:removeItem(2667, 5) then
				npcHandler:say("Excellent, now bring me 5 northern pike.", cid)
				player:setStorageValue(PlayerStorageKeys.CaptainHaba, 3)
				npcHandler.topic[cid] = 3
			end
		elseif player:getStorageValue(PlayerStorageKeys.CaptainHaba) == 3 then
			if player:removeItem(2669, 5) then
				npcHandler:say("Excellent, now bring me 5 green perch.", cid)
				player:setStorageValue(PlayerStorageKeys.CaptainHaba, 4)
				npcHandler.topic[cid] = 4
			end
		elseif player:getStorageValue(PlayerStorageKeys.CaptainHaba) == 4 then
			if player:removeItem(7159, 5) then
				npcHandler:say("Excellent, now bring me 5 rainbow trout.", cid)
				player:setStorageValue(PlayerStorageKeys.CaptainHaba, 5)
				npcHandler.topic[cid] = 5
			end
		elseif player:getStorageValue(PlayerStorageKeys.CaptainHaba) == 5 then
			if player:removeItem(7158, 5) then
				...
				player:setStorageValue(PlayerStorageKeys.CaptainHaba, 6)
				npcHandler.topic[cid] = 6
			end
		end
	elseif msgcontains(msg, "hunt") then
		if player:getStorageValue(PlayerStorageKeys.CaptainHaba) == 6 then
