...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'key') then
		if player:getStorageValue(PlayerStorageKeys.thievesGuild.Mission06) == 1 then
			local headItem = player:getSlotItem(CONST_SLOT_HEAD)
			if headItem and headItem.itemid == 2665 and player:getStorageValue(PlayerStorageKeys.postman.Rank) == 5 and player:getSex() ~= PLAYERSEX_FEMALE then
				player:addItem(8762)
				player:setStorageValue(PlayerStorageKeys.thievesGuild.Mission06, 2)
