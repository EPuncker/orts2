...
local function creatureSayCallback(cid, type, msg)
	...
	local votingPoints = player:getStorageValue(PlayerStorageKeys.Oramond.VotingPoints)
	-- START TASK
	if msgcontains(msg, "food") then
		if player:getStorageValue(PlayerStorageKeys.Oramond.MissionToTakeRoots) < 0 then
			...
			player:setStorageValue(PlayerStorageKeys.Oramond.HarvestedRootCount, 0)
	...
	if(msgcontains(msg, "yes")) then
		if npcHandler.topic[cid] == 1 and player:getStorageValue(PlayerStorageKeys.Oramond.HarvestedRootCount) >= 5 then
			npcHandler:say("Come when you are done.", cid)
			npcHandler.topic[cid] = 0
		elseif npcHandler.topic[cid] == 2 and player:getStorageValue(PlayerStorageKeys.Oramond.HarvestedRootCount) >= 5 then
			npcHandler:say("Spend it wisley, thought, put in a word for the poor, will ye? Sure you will. ", cid)
			npcHandler.topic[cid] = 3
		elseif npcHandler.topic[cid] == 3 then
			player:setStorageValue(PlayerStorageKeys.Oramond.VotingPoints, votingPoints + 1)
			player:setStorageValue(PlayerStorageKeys.Oramond.HarvestedRootCount, harvestedCount - 5)
