...
local function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "mission")) then
		if(player:getStorageValue(PlayerStorageKeys.TheNewFrontier.Questline) == 17) then
		...
		elseif(player:getStorageValue(PlayerStorageKeys.TheNewFrontier.Questline) == 19) then
			npcHandler:say({
			...
			player:setStorageValue(PlayerStorageKeys.TheNewFrontier.Questline, 20)
			player:setStorageValue(PlayerStorageKeys.TheNewFrontier.Mission06, 3) --Questlog, The New Frontier Quest "Mission 06: Days Of Doom"
		end
	elseif(msgcontains(msg, "test")) then
		if(npcHandler.topic[cid] == 1) then
			...
			player:setStorageValue(PlayerStorageKeys.TheNewFrontier.Questline, 18)
			player:setStorageValue(PlayerStorageKeys.TheNewFrontier.Mission06, 2) --Questlog, The New Frontier Quest "Mission 06: Days Of Doom"
		end
	end
	return true
end
