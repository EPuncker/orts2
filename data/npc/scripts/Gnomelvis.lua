...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "looking") then
		if player:getStorageValue(PlayerStorageKeys.BigfootBurden.QuestLine) == 11 then
			npcHandler:say("I'm the gnomish musical supervisor!", cid)
			npcHandler.topic[cid] = 1
		elseif player:getStorageValue(PlayerStorageKeys.BigfootBurden.QuestLine) == 13 then
			npcHandler:say("I'm the gnomish musical supervisor!", cid)
	...
	elseif msgcontains(msg, "musical") then
		if npcHandler.topic[cid] == 1 then
			...
			for i = 0, 6 do
				player:setStorageValue(PlayerStorageKeys.BigfootBurden.MelodyTone1 + i, math.random(3124, 3127))
			end
		elseif npcHandler.topic[cid] == 2 then
			...
			player:setStorageValue(PlayerStorageKeys.BigfootBurden.QuestLine, 14)
			player:setStorageValue(PlayerStorageKeys.BigfootBurden.Rank)
