...
local function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "addon")) then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanFirstAddon) < 1 then
		...
		elseif player:getStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanSecondAddon) < 1 then
			npcHandler:say("Currently we are offering accessories for the nobleman - and, of course, noblewoman - outfit. Would you like to hear more about our offer?", cid)
			npcHandler.topic[cid] = 1
		else
	...
	elseif(msgcontains(msg, "yes")) then
		...
		elseif npcHandler.topic[cid] == 3 then
			if player:getTotalMoney() >= 150000 and player:getStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanFirstAddon) < 1 then
				...
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanFirstAddon, 1)
				player:removeTotalMoney(150000)
				npcHandler.topic[cid] = 0
			end
		elseif npcHandler.topic[cid] == 4 then
			if player:getTotalMoney() >= 150000 and player:getStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanSecondAddon) < 1 then
				npcHandler:say("Congratulations! Here is your brand-new accessory, I hope you like it. Please visit us again!", cid)
				player:addOutfitAddon(140, 1)
				player:addOutfitAddon(132, 1)
				player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanSecondAddon, 1)
				player:removeTotalMoney(150000)
				npcHandler.topic[cid] = 0
			end
		end
	elseif(msgcontains(msg, "hat") or msgcontains(msg, "accessory")) and (npcHandler.topic[cid] == 2 and player:getStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanFirstAddon) < 1) then
		npcHandler:say({
	...
	elseif(msgcontains(msg, "dress") or msgcontains(msg, "coat")) and (npcHandler.topic[cid] == 2 and player:getStorageValue(PlayerStorageKeys.OutfitQuest.NoblemanSecondAddon) < 1) then
		npcHandler:say({
			"This accessory requires a small fee of 150000 gold pieces. Of course, we do not want to put you at any risk to be attacked while carrying this huge amount of money. ...",
			"This is why we have established our brand-new instalment sale. You can choose to either pay the price at once, or if you want to be safe, by instalments of 10000 gold pieces. ...",
			"I also have to inform you that once you started paying for one of the accessories, you have to finish the payment first before you can start paying for the other one, of course. ...",
