...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "trouble") and player:getStorageValue(PlayerStorageKeys.TheInquisition.KulagGuard) < 1 and player:getStorageValue(PlayerStorageKeys.TheInquisition.Mission01) ~= -1 then
	...
	elseif msgcontains(msg, "authorities") then
		if npcHandler.topic[cid] == 1 then
			...
			if player:getStorageValue(PlayerStorageKeys.TheInquisition.KulagGuard) < 1 then
				player:setStorageValue(PlayerStorageKeys.TheInquisition.KulagGuard, 1)
