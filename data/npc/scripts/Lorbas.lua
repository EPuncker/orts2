...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'cookie') then
		if player:getStorageValue(PlayerStorageKeys.WhatAFoolishQuest.Questline) == 31
				and player:getStorageValue(PlayerStorageKeys.WhatAFoolishQuest.CookieDelivery.Lorbas) ~= 1 then
	...
	elseif msgcontains(msg, 'yes') then
		if npcHandler.topic[cid] == 1 then
			...
			player:setStorageValue(PlayerStorageKeys.WhatAFoolishQuest.CookieDelivery.Lorbas, 1)
