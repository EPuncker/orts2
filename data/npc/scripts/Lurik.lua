...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		if player:getStorageValue(PlayerStorageKeys.ExplorerSociety.QuestLine) == 61 then
			npcHandler:say({
			...
			player:setStorageValue(PlayerStorageKeys.ExplorerSociety.QuestLine, 62)
		elseif player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) == 32 then
			npcHandler:say({
		...
		elseif player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) == 33 then
			npcHandler:say("Have you retrieved the memory crystal?", cid)
			npcHandler.topic[cid] = 2
		elseif player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) == 34 and player:getStorageValue(PlayerStorageKeys.TheIceIslands.MemoryCrystal) > os.time() then
		...
		elseif player:getStorageValue(PlayerStorageKeys.TheIceIslands.Questline) == 34 and player:getStorageValue(PlayerStorageKeys.TheIceIslands.MemoryCrystal) < os.time() then
			...
			npcHandler.topic[cid] = 0
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Questline, 35)
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Mission09, 1) -- Questlog The Ice Islands Quest, Formorgar Mines 1: The Mission
		end
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			npcHandler:say("Excellent. Just report about your mission when you got the memory crystal.", cid)
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Questline, 33)
			player:setStorageValue(PlayerStorageKeys.TheIceIslands.Mission08, 2) -- Questlog The Ice Islands Quest, The Contact
			npcHandler.topic[cid] = 0
		elseif npcHandler.topic[cid] == 2 then
			if player:removeItem(7281, 1) then
				...
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.Questline, 34)
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.Mission08, 4) -- Questlog The Ice Islands Quest, The Contact
				player:setStorageValue(PlayerStorageKeys.TheIceIslands.MemoryCrystal, os.time() + 5 * 60)
				npcHandler.topic[cid] = 0
			end
		end
	end
