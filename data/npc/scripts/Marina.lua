...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "silk") or msgcontains(msg, "yarn") then
		if player:getStorageValue(PlayerStorageKeys.FriendsandTraders.TheMermaidMarina) < 1 then
		...
		elseif player:getStorageValue(PlayerStorageKeys.FriendsandTraders.TheMermaidMarina) == 2 then
	...
	elseif msgcontains(msg, "raymond striker") then
		if player:getStorageValue(PlayerStorageKeys.TheShatteredIsles.APoemForTheMermaid) == 1 then
			...
			player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.APoemForTheMermaid, 2)
			npcHandler.topic[cid] = 0
		end
	elseif msgcontains(msg, "date") then
		if player:getStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove) == 1 then
			npcHandler:say("Is that the best you can do? A true Djinn would have done something more poetic.", cid)
			player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove, 2)
			npcHandler.topic[cid] = 0
		elseif player:getStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove) == 4 then
			npcHandler:say({
			...
			player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove, 5)
			player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.AccessToLagunaIsland, 1)
			npcHandler.topic[cid] = 0
		end
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[cid] == 2 then
			...
			player:setStorageValue(PlayerStorageKeys.FriendsandTraders.TheMermaidMarina, 1)
		...
		elseif npcHandler.topic[cid] == 4 then
			if player:removeItem(5902, 50) then
				...
				npcHandler.topic[cid] = 0
				player:setStorageValue(PlayerStorageKeys.FriendsandTraders.TheMermaidMarina, 2)
			else
