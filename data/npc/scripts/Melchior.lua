...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'words of greeting') then
		if player:getStorageValue(PlayerStorageKeys.DjinnWar.Faction.Greeting) ~= 0 then
			npcHandler:say({
			...
			if player:getStorageValue(PlayerStorageKeys.Factions) ~= 1 then
				player:setStorageValue(PlayerStorageKeys.Factions, 1)
			end
			player:setStorageValue(PlayerStorageKeys.DjinnWar.Faction.Greeting, 1)
		end
	end
