...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "eleonore") then
		if player:getStorageValue(PlayerStorageKeys.TheShatteredIsles.APoemForTheMermaid) == 2 and player:getStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove) < 1 then
	...
	elseif msgcontains(msg, "mermaid") or msgcontains(msg, "marina") then
		...
		elseif player:getStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove) == 2 then
			...
			player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove, 3)
		end
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 3 then
			...
			player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove, 1)
		elseif npcHandler.topic[cid] == 4 then
			if player:removeItem(5952, 1) then
				...
				player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove, 4)
				player:setStorageValue(PlayerStorageKeys.TheShatteredIsles.APoemForTheMermaid, 3)
				npcHandler.topic[cid] = 0
			else
				npcHandler.topic[cid] = 0
	...
	elseif msgcontains(msg, "love poem") then
		if player:getStorageValue(PlayerStorageKeys.TheShatteredIsles.ADjinnInLove) == 3 then
