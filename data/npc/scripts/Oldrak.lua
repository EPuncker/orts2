...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'mission') or msgcontains(msg, 'demon oak') then
		if player:getStorageValue(PlayerStorageKeys.DemonOak.Done) < 1 and player:getStorageValue(PlayerStorageKeys.DemonOak.Progress) < 2 then
			npcHandler:say("How do you know? Did you go into the infested area?", cid)
			npcHandler.topic[cid] = 1
		elseif player:getStorageValue(PlayerStorageKeys.DemonOak.Progress) == 2 and player:getStorageValue(PlayerStorageKeys.DemonOak.Done) < 1 then
			npcHandler:say("You better don't return here until you've defeated the Demon Oak.", cid)
		elseif player:getStorageValue(PlayerStorageKeys.DemonOak.Done) == 1 then
			npcHandler:say({
			...
			player:setStorageValue(PlayerStorageKeys.DemonOak.Done, 2)
		end
	elseif msgcontains(msg, 'yes') and npcHandler.topic[cid] == 1 then
		if player:getStorageValue(PlayerStorageKeys.DemonOak.Progress) == 1 then
			...
			player:setStorageValue(PlayerStorageKeys.DemonOak.Progress, 2)
	...
	elseif msgcontains(msg, 'axe') then
		if player:getStorageValue(PlayerStorageKeys.DemonOak.Progress) == 2 then
	...
	elseif msgcontains(msg, 'yes') and npcHandler.topic[cid] == 2 then
		if player:getStorageValue(PlayerStorageKeys.DemonOak.Progress) == 2 then
			if player:getTotalMoney() >= 1000 then
				if player:removeItem(2386, 1) and player:removeTotalMoney(1000) then
