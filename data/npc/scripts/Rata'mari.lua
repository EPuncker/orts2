...
local function greetCallback(cid)
	if Player(cid):getStorageValue(PlayerStorageKeys.DjinnWar.MaridFaction.Mission02) == -1 then
		return false
	end
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'spy report') then
		local reportProgress = player:getStorageValue(PlayerStorageKeys.DjinnWar.MaridFaction.RataMari)
		if reportProgress < 1 then
			...
			player:setStorageValue(PlayerStorageKeys.DjinnWar.MaridFaction.RataMari, 1)
	...
	elseif npcHandler.topic[cid] == 1 then
		if msgcontains(msg, 'yes') then
			...
			player:setStorageValue(PlayerStorageKeys.DjinnWar.MaridFaction.RataMari, 2)
