...
keywordHandler:addKeyword({'help'}, StdModule.say, {npcHandler = npcHandler, text = "{Slime}. Dangerous. We. Have. It. Under. Control. ... We. Will. Stand. {Sentry}."}, function(player) return player:getStorageValue(PlayerStorageKeys.TheirMastersVoice.SlimeGobblerReceived) == 1 end)
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "help") then
		if player:getStorageValue(PlayerStorageKeys.TheirMastersVoice.SlimeGobblerReceived) < 1 then
			npcHandler:say("Defeat. {Slime}. We. Will. Why. Did. You. Kill. Us? Do. You. Want. To. Rectify. And. Help?", cid)
			npcHandler.topic[cid] = 1
		end
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			player:setStorageValue(PlayerStorageKeys.TheirMastersVoice.SlimeGobblerReceived, 1)
