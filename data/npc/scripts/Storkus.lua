...
local function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "mission")) then
		if(player:getStorageValue(PlayerStorageKeys.TheInquisition.Questline) == 6) then
			if(player:getStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust) < 0) then
				player:setStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust, 0)
			end
			if(player:getStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust) < 20) then
				npcHandler:say("So far ye've brought me " .. player:getItemCount(5905) .. " of 20 {vampire dusts}. Do ye' have any more with ye'? ", cid)
				npcHandler.topic[cid] = 1
			elseif(player:getStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust) == 20) then
				npcHandler:say("Fine, you're done! Ye' should talk to me about your {mission} again now.", cid)
				npcHandler.topic[cid] = 2
				player:setStorageValue(PlayerStorageKeys.TheInquisition.Questline, 7)
				player:setStorageValue(PlayerStorageKeys.TheInquisition.Mission03, 2) -- The Inquisition Questlog- "Mission 3: Vampire Hunt"
			end
		elseif(player:getStorageValue(PlayerStorageKeys.TheInquisition.Questline) == 7) then
			npcHandler:say({
			...
			player:setStorageValue(PlayerStorageKeys.TheInquisition.Questline, 8)
			player:setStorageValue(PlayerStorageKeys.TheInquisition.Mission03, 3) -- The Inquisition Questlog- "Mission 3: Vampire Hunt"
			npcHandler.topic[cid] = 0
		elseif(player:getStorageValue(PlayerStorageKeys.TheInquisition.Questline) == 8 or player:getStorageValue(PlayerStorageKeys.TheInquisition.Questline) == 9) then
			if(player:removeItem(8752, 1)) then
				...
				player:setStorageValue(PlayerStorageKeys.TheInquisition.Questline, 10)
				player:setStorageValue(PlayerStorageKeys.TheInquisition.Mission03, 5) -- The Inquisition Questlog- "Mission 3: Vampire Hunt"
			else
				npcHandler:say("Have ye' killed the vampire lord? Because ye' have no his ring.", cid)
			end
			npcHandler.topic[cid] = 0
		end
	elseif(msgcontains(msg, "vampire lord token") and player:getStorageValue(PlayerStorageKeys.TheInquisition.Questline) == 10) then
		if(player:getStorageValue(PlayerStorageKeys.VampireHunter.Rank) < 1) then
			npcHandler:say("Would ye' like to give me vampire tokens?", cid)
			npcHandler.topic[cid] = 3
		elseif(player:getStorageValue(PlayerStorageKeys.VampireHunter.Rank) == 1) then
			npcHandler:say("Would ye' like to give me vampire tokens?", cid)
			npcHandler.topic[cid] = 4
		elseif(player:getStorageValue(PlayerStorageKeys.VampireHunter.Rank) == 2) then
			npcHandler:say("Would ye' like to give me vampire tokens?", cid)
			npcHandler.topic[cid] = 5
		elseif(player:getStorageValue(PlayerStorageKeys.VampireHunter.Rank) == 3) then
			npcHandler:say("Would ye' like to give me vampire tokens?", cid)
			npcHandler.topic[cid] = 6
		elseif(player:getStorageValue(PlayerStorageKeys.VampireHunter.Rank) == 4) then
			npcHandler:say("Would ye' like to give me vampire tokens?", cid)
			npcHandler.topic[cid] = 7
		elseif(player:getStorageValue(PlayerStorageKeys.VampireHunter.Rank) == 5) then
			npcHandler:say("Would ye' like to give me vampire tokens?", cid)
	...
	elseif(msgcontains(msg, "yes")) then
		if(npcHandler.topic[cid] == 1) then
			...
			requiredCount = 20 - player:getStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust)
			if(count > requiredCount) then
				count = requiredCount
			end
			player:setStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust, player:getStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust) + count)
			player:removeItem(5905, count)
			npcHandler:say("Ye've brought me " .. count .. " vampire dusts. " .. (20 - player:getStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust)) == 0 and ("Ask me for a {mission} to continue your quest.") or ("Ye' need to bring " .. (20 - player:getStorageValue(PlayerStorageKeys.TheInquisition.StorkusVampiredust)) .. " more."), cid)
			npcHandler.topic[cid] = 0
		elseif(npcHandler.topic[cid] == 3) then
			if player:removeItem(9020, 1) then
				...
				player:setStorageValue(PlayerStorageKeys.VampireHunter.Rank, 1)
		...
		elseif(npcHandler.topic[cid] == 4) then
			if player:removeItem(9020, 4) then
				...
				player:setStorageValue(PlayerStorageKeys.VampireHunter.Rank, 2)
		...
		elseif(npcHandler.topic[cid] == 5) then
			if player:removeItem(9020, 5) then
				...
				player:setStorageValue(PlayerStorageKeys.VampireHunter.Rank, 3)
		...
		elseif(npcHandler.topic[cid] == 6) then
			if player:removeItem(9020, 10) then
				...
				player:setStorageValue(PlayerStorageKeys.VampireHunter.Rank, 4)
		...
		elseif(npcHandler.topic[cid] == 7) then
			if player:removeItem(9020, 30) then
				...
				player:setStorageValue(PlayerStorageKeys.VampireHunter.Rank, 5)
		...
		elseif(npcHandler.topic[cid] == 8) then
			if player:removeItem(9020, 50) then
				...
				player:setStorageValue(PlayerStorageKeys.VampireHunter.Rank, 6)
