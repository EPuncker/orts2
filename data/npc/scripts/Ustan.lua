...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "addon") then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.DruidBodyAddon) < 1 then
			...
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.DruidBodyAddon, 1)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			if player:getItemCount(5896) >= 50 and player:getItemCount(5897) >= 50 then
				...
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.DruidBodyAddon, 2)
