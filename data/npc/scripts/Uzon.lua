...
addTravelKeyword('eclipse', 'Oh no, so the time has come? Do you really want me to fly you to this unholy place?', 0, Position(32659, 31915, 0), function(player) return player:getStorageValue(PlayerStorageKeys.TheInquisition.Questline) ~= 4 end)
addTravelKeyword('farmine', 'Do you seek a ride to Farmine for |TRAVELCOST|?', 60, Position(32983, 31539, 1), function(player) return player:getStorageValue(PlayerStorageKeys.TheNewFrontier.Mission10) ~= 1 end)
addTravelKeyword('edron', 'Do you seek a ride to Edron for |TRAVELCOST|?', 60, Position(33193, 31783, 3), nil, function(player) if player:getStorageValue(PlayerStorageKeys.postman.Mission01) == 2 then player:setStorageValue(PlayerStorageKeys.postman.Mission01, 3) end end)
