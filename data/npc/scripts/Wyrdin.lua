...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		if player:getStorageValue(PlayerStorageKeys.TheWayToYalahar.QuestLine) < 1 and player:getStorageValue(PlayerStorageKeys.ExplorerSociety.QuestLine) >= 4 then
			npcHandler:say({
			...
			player:setStorageValue(PlayerStorageKeys.TheWayToYalahar.QuestLine, 1)
			npcHandler.topic[cid] = 0
		elseif player:getStorageValue(PlayerStorageKeys.TheWayToYalahar.QuestLine) == 2 then
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			if player:removeItem(10090, 1) then
				player:setStorageValue(PlayerStorageKeys.TheWayToYalahar.QuestLine, 3)
	...
	elseif msgcontains(msg, "plea") then
		if npcHandler.topic[cid] == 2 then
			if player:getStorageValue(PlayerStorageKeys.TheNewFrontier.BribeWydrin) < 1 then
				...
				player:setStorageValue(PlayerStorageKeys.TheNewFrontier.BribeWydrin, 1)
				player:setStorageValue(PlayerStorageKeys.TheNewFrontier.Mission05, player:getStorageValue(PlayerStorageKeys.TheNewFrontier.Mission05) + 1) --Questlog, The New Frontier Quest "Mission 05: Getting Things Busy"
			end
		end
	end
	return true
