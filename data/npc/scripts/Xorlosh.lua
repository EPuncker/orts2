...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		if player:getStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.GoingDown) < 1 then
		...
		elseif player:getStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.GoingDown) == 1 and player:removeItem(9690, 3) then
			player:setStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.GoingDown, 2)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			player:setStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.GoingDown, 1)
